...
const Header = ({toggleFunc}) => {
  ...
  return (
    <nav className="h-16 top-0  mx-auto fixed z-50 w-full bg-primary text-secondary/90 dark:text-secondary/90 dark:bg-offblack dark:border-b dark:border-offblack/60 md:px-15 px-5" >
      ...
      <ul className="relative z-50 gap-4 hidden md:flex ">
        <li ><a aria-label="goto home page" href="#home"  className="text-shadow-xs px-2 py-2 rounded-md hover:text-white dark:hover:hover:text-white transition-all duration-100 ease-in leading-4 hover:underline-offset-6 hover:underline decoration-white dark:decoration-white" >Home</a></li>
      ...
      <ul className={`${showNavigation ? 'translate-x-0' : ''} transition -translate-x-full absolute z-10 gap-4 md:hidden h-[calc(100vh - 64px)] w-full top-16 left-0 bg-white dark:bg-offblack text-black/80 dark:text-white/80 pt-4 px-4 flex flex-col py-8 border-b border-slate-300/50`}>
        <li ><a aria-label="Home" href="#home" className="text-shadow-xs px-2 py-2 rounded-md hover:text-primary/70 leading-4 underline-offset-6 hover:underline decoration-primary" >Home</a></li>
